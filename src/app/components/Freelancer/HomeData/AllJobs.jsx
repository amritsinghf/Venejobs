  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {jobs?.map((item) => (
        <div
          key={item.id}
          className="rounded-xl border border-[rgba(68,68,68,0.08)] bg-white p-6 flex flex-col gap-4"
        >
          ...
          <div className="flex flex-wrap gap-2">
            {item.skills?.map((skill, index) => (
              <span
                key={index}
                className="px-3 py-1 text-sm bg-gray-100 rounded-full text-heading font-medium"
              >
                {skill.name}
